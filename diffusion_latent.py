...
class Asyrp(object):
    ...
    def run_training(self):
        ...
        print(f'seq_test : {seq_test.shape}. seq_test_next : {seq_test_next}')

        # ----------- Model -----------#
        model = self.load_pretrained_model()
        print(f'load pretrained model : {model}')
        optim_param_list = []
        delta_h_dict = {}
        for i in seq_train:
            delta_h_dict[i] = None
        ...
        print(f'optim_param_list : {optim_param_list.shape}')
        ...
        print(f'img_lat_pais_dic -> test : {np.shape(img_lat_pairs_dic["test"])}, train : {np.shape(img_lat_pairs_dic["train"])}')
            ...
            if self.args.do_train:
                ...
                if self.args.retrain==0 and os.path.exists(save_name):
                    ...
                    for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic['train']):
                        ...
                        print(f'x_origin : {x_origin.shape}, xt_next : {xt_next.shape}')
                        ...
                        with tqdm(total=len(seq_train), desc=f"training iteration") as progress_bar:
                            for t_it, (i, j) in enumerate(zip(reversed(seq_train), reversed(seq_train_next))):
                                ...
                                print(f'xt_next : {xt_next.shape}, x0_t : {x0_t.shape}')
                                ...
                                print(f'x_origin : {x_origin.shape}, x0_t_origin : {x0_t_origin.shape}')
    ...
    def save_image(self, model, x_lat_tensor, seq_inv, seq_inv_next,
        ...
        print(f'save_image start : x_lat_tensor : {x_lat_tensor.shape}, seq_inv : {seq_inv.shape}, seq_inv_next : {seq_inv_next.shape}')
        ...
        with tqdm(total=len(seq_inv)*(process_num), desc=f"Generative process") as progress_bar:
            ...
            if self.args.pass_editing:
                ...
                for hs_coeff_tuple in hs_coeff:
                    print(f'hs_coeff_tuple : {hs_coeff_tuple}')
                    ...
                    for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                        ...
                        progress_bar.update(1)
                        print(f'x : {x.shape}, x0_t : {x0_t.shape}, delta_h : {delta_h.shape}')
    ...
    def run_test(self):
        ...
        print(f'seq_test : {seq_test}, seq_test_edit : {seq_test_edit}, seq_test_next : {seq_test_next}')
        ...
        print(f'save_name_list : {save_name_list}, hs_coeff : {hs_coeff}')
        ...
        if os.path.exists(save_name_list[0]):
            ...
            if self.args.train_delta_block:
                print('self.args.train_delta_block')
            ...
            print('save_name_list[0] does not exist')
            if self.args.num_mean_of_delta_hs:
        ...
        if self.args.n_train_step != self.args.n_test_step:
            print('n_train_step != n_test_step')
            if self.args.train_delta_h:
                trained_idx = 0
                test_delta_h_dict = {}
                if self.args.ignore_timesteps:
        ...
        if self.args.delta_interpolation:
            print('self.args.delta_interpolation')
            if self.args.multiple_attr:
                print('self.args.multiple_attr')
        ...
        # Train set
        print("Train set!!")
        print(f'img_lat_pairs_dic : {img_lat_pairs_dic}')
        if self.args.do_train:
            for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic['train']):
                print(f'x0 : {x0.shape}, x_lat : {x_lat.shape}')
                if self.args.target_image_id:
                    assert self.args.bs_train == 1, "target_image_id is only supported for batch_size == 1"
                    if not step in self.args.target_image_id:
                        continue
            ...
            if os.path.exists(pairs_path) and not self.args.re_precompute:
                ...
                for exist_precompute_num in reversed(range(self.args.n_train_img if mode == 'train' else self.args.n_test_img)):
                    ...
                    if os.path.exists(tmp_path):
                        ...
                        break
                print(self.config.data.category)
                if self.config.data.category == 'CUSTOM':
                    print('data category IS CUSTOM')
    ...
    def set_t_edit_t_addnoise(self, LPIPS_th=0.33, LPIPS_addnoise_th=0.1, return_clip_loss=False):
        ...
        if self.args.user_defined_t_edit:
            print(f'user_defined_t_edit : {self.args.user_defined_t_edit}')
        ...
        if self.args.user_defined_t_addnoise:
            print(f'self.args.user_defined_t_addnoise : {self.args.user_defined_t_addnoise}')
